package t1_array.s3_hard.q16_containsDuplicate3;
...
class Solution {
    ...
    public boolean containsNearbyAlmostDuplicate(int[] nums, int indexDiff, int valueDiff) {
        if (valueDiff < 0 || indexDiff < 0 || nums.length < 2)
            return false;
        ...
        for (int i = 0; i < nums.length; ++i) {
            ...
            // check if bucket bucketId is empty, each bucket may contain at most one
            // element
            ...
            if (i >= indexDiff)
                bucket.remove(getBucketId(nums[i - indexDiff], width));
        }
        return false;
    }
}
